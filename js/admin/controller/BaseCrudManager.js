...
import { serializeForm, showErrors, resetFormState } from '../../utils/admin-form.util.js';
...
export class BaseCrudManager {
    ...
    init(container) {
        ...
        this.dataTable = new DataTable(container, {
            columns: columns,
            searchPlaceholder: tableOptions.searchPlaceholder || `Search ${entityName}...`,
            actionBtnText: tableOptions.actionBtnText || `+ Create ${entityName}`,
            fetchData: async (params) => {
                const result = params.tab === 'trashed'
    ...
    async handleTableAction(action, dataset) {
        ...
        switch (action) {
            ...
            case 'delete':
                this.dataTable.showConfirm(`Move "${dataset.name}" to trash?`, async () => {
            ...
            case 'force-delete':
                this.dataTable.showConfirm(`Permanently delete "${dataset.name}"?`, async () => {
    ...
    async handleFormSubmit(e) {
        ...
        if (this.currentMode === 'create') {
            ...
            const updateData = this.config.formatUpdateData ? this.config.formatUpdateData(data) : data;
            result = await this.config.service.update(this.currentId, updateData);
        }

        this.modal.setLoading(false, 'Saving…', this.currentMode === 'create' ? 'Submit' : 'Save Changes');
